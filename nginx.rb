...
class NginxHttpPushModule < Formula
  homepage 'https://github.com/slact/nginx_http_push_module'
  url 'git://github.com/slact/nginx_http_push_module.git', :using => :git
  def initialize; super 'NginxHttpPushModule'; end
end
...
class Nginx < Formula
  ...
  option 'with-hpm', 'Compile with support for HTTP Push module'
  ...
  def http_push_args
    NginxHttpPushModule.new.brew { (buildpath + 'nginx_http_push_module').install Dir['*'] }
    "--add-module=nginx_http_push_module"
  end
  ...
  def install
    ...
    args << http_push_args if build.with? 'hpm'
